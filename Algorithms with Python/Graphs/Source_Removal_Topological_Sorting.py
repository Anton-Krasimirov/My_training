...
def find_node_without_dependecies(dependencies_by_node):
    for node, dependencies in dependencies_by_node.items():
        if dependencies == 0:
            return node
    return None# означава че някаде в графа има цикъл и по правило не можем да го обходим с топографско сортиране
...
while dependencies_by_node:
    node_to_remove = find_node_without_dependecies(dependencies_by_node)
